"""
QUANTUM BRAIN - BLUEGUARDIAN DEDICATED
=======================================
Handles BlueGuardian accounts ONLY.

Accounts:
  - 366604 ($5K Instant) - ACTIVE
  - 365060 ($100K Challenge) - LOCKED until 5K passes

Run: python BRAIN_BLUEGUARDIAN.py
     python BRAIN_BLUEGUARDIAN.py --unlock-all
...
logging.basicConfig(
    level=logging.INFO,
    format='[%(asctime)s][BG] %(message)s',
    datefmt='%H:%M:%S',
    handlers=[
        logging.FileHandler('brain_blueguardian.log'),
        logging.StreamHandler()
    ]
)

# ============================================================
# BLUEGUARDIAN ACCOUNTS ONLY
# ============================================================
...
ACCOUNTS = {
    ...
    'BG_100K_CHALLENGE': {
        ...
        'daily_loss_limit': 0.05,
        'max_drawdown': 0.10,
        'locked': True,  # Unlock after 5K passes
        'symbols': ['BTCUSD', 'ETHUSD'],
        'magic_number': 365001,
    },
}
...
class AccountTrader:
    def __init__(self, account_key: str, account_config: dict, config: BrainConfig):
        ...
        self.partial_closed_tickets = set()  # Track tickets we've already partial closed
    ...
    def manage_positions(self):
        """
        50% PARTIAL CLOSE STRATEGY:
        - When position reaches 50% of TP distance, close half
        - Move SL to breakeven
        - Let the rest run to full TP
        """
        ...
        for pos in positions:
            ...
            # Skip if already partial closed
            ...
            if pos_type == mt5.POSITION_TYPE_BUY:
                tp_distance = tp - open_price
                current_profit_distance = current_price - open_price
            else:  # SELL
                tp_distance = open_price - tp
                current_profit_distance = open_price - current_price

            # Check if we've reached 50% of TP
            if tp_distance > 0 and current_profit_distance >= (tp_distance * 0.5):
                logging.info(f"[{self.account_key}] Position {ticket} reached 50% TP - PARTIAL CLOSE")
                ...
                # Calculate half volume (respecting volume step)
                half_volume = volume / 2
                half_volume = round(half_volume / symbol_info.volume_step) * symbol_info.volume_step
                half_volume = max(symbol_info.volume_min, half_volume)

                # Can only partial close if remaining volume is valid
                remaining = volume - half_volume
                if remaining < symbol_info.volume_min:
                    logging.info(f"[{self.account_key}] Volume too small for partial close, skipping")
                    continue
                ...
                if result is None:
                    logging.error(f"[{self.account_key}] Partial close failed - None result")
                    continue

                if result.retcode == mt5.TRADE_RETCODE_DONE:
                    logging.info(f"[{self.account_key}] PARTIAL CLOSE: {half_volume} lots @ {close_price:.2f}")
                    self.partial_closed_tickets.add(ticket)

                    # Move SL to breakeven for remaining position
                    # Need to get the new ticket (partial close creates new position)
                    new_positions = mt5.positions_get(symbol=symbol)
                    if new_positions:
                        for new_pos in new_positions:
                            if new_pos.magic == self.account_config['magic_number']:
                                # Modify SL to breakeven
                                modify_request = {
                                    "action": mt5.TRADE_ACTION_SLTP,
                                    "symbol": symbol,
                                    "position": new_pos.ticket,
                                    "sl": open_price,  # Breakeven
                                    "tp": tp,  # Keep original TP
                                }
                                mod_result = mt5.order_send(modify_request)
                                if mod_result and mod_result.retcode == mt5.TRADE_RETCODE_DONE:
                                    logging.info(f"[{self.account_key}] SL moved to breakeven @ {open_price:.2f}")
                                else:
                                    logging.warning(f"[{self.account_key}] Failed to move SL to breakeven")
                else:
                    logging.error(f"[{self.account_key}] Partial close failed: {result.comment}")
    ...
    def execute_trade(self, symbol: str, action: Action, confidence: float) -> bool:
        ...
        # DYNAMIC SL/TP: Target $0.50 loss, 3:1 ratio ($1.50 profit)
        # Calculate SL distance based on lot size to achieve $0.50 risk
        target_loss = 0.50   # $0.50 max loss per trade
        ratio = 3.0          # 3:1 reward to risk

        # For crypto: contract size and tick value vary
        # ETHUSD: 0.1 lot, $1 move = ~$0.10 P/L, so $5 move = $0.50
        # BTCUSD: 0.01 lot, $1 move = ~$0.01 P/L, so $50 move = $0.50
        if 'ETH' in symbol:
            sl_distance = 5.0    # $5 price move = ~$0.50 loss at 0.1 lot
        elif 'BTC' in symbol:
            sl_distance = 50.0   # $50 price move = ~$0.50 loss at 0.01 lot
        else:
            sl_distance = 5.0

        tp_distance = sl_distance * ratio  # 3:1 ratio
        ...
        request = {
            ...
            "tp": tp,
            "magic": self.account_config['magic_number'],
            "comment": f"BG_{self.account_key}",
            "type_time": mt5.ORDER_TIME_GTC,
    ...
    def run_cycle(self) -> Dict[str, dict]:
        ...
        # Manage existing positions (50% partial close)
...
class BlueGuardianBrain:
    ...
    def run_loop(self):
        print("=" * 60)
        print("  BLUEGUARDIAN QUANTUM BRAIN")
        print("  Dedicated brain for BlueGuardian accounts")
        print("=" * 60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='BlueGuardian Quantum Brain')
    parser.add_argument('--unlock-all', action='store_true', help='Trade all BG accounts')
    args = parser.parse_args()

    brain = BlueGuardianBrain(unlock_all=args.unlock_all)
    brain.run_loop()
