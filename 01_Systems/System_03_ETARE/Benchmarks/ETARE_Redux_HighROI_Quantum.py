...
SYMBOLS = [
    ...
    "AUDNZD", "AUDCHF", "NZDCHF", "NZDCAD", "CADCHF", "AUDCAD",
    # Commodities & Crypto
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        # Hybrid execution: Run LSTM on CPU, rest on GPU/Device
        # x is likely on DEVICE
        x_cpu = x.cpu()
...
class TradingIndividual:
    def __init__(self, input_size: int, hidden_size: int = 128, quantum_seed=None):
        ...
        # DirectML Fix: Force LSTM layer to CPU to avoid fused kernel crash
    ...
    def evaluate_population(self, df, deposit):
        ...
        for individual in self.population:
            ...
            with torch.no_grad():
                ...
                individual.fitness = (final_value - deposit) / deposit # ROI as fitness
