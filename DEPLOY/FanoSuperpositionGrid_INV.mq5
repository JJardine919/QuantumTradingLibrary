...
//|      Fano Plane Octonion Superposition Grid — INVERTED MODE      |
//|                                 QuantumChildren / DooDoo 2026    |
//|                                                                  |
//| Identical to FanoSuperpositionGrid.mq5 but with InvertedMode     |
//| defaulting to TRUE and a separate MagicNumber (212002).          |
//| Run alongside the normal copy for dual-EA coverage.              |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
input group "=== ACCOUNT ==="
input int      MagicNumber         = 212002;       // Magic Number (INV)
input string   AccountName         = "FANO_INV";   // Account Name
...
input bool     InvertedMode        = true;         // Invert regime signal (DEFAULT: ON)
...
{
   Print("================================================================");
   Print("  FANO SUPERPOSITION GRID EA v1.0 [INVERTED]");
   ...
   Print("FANO EA [INV] initialized. Grid synced: ", g_grid.count, " positions.");
...
void OnDeinit(const int reason)
{
   ...
   Print("FANO EA [INV] deinitialized. Reason: ", reason);
}
...
void OnTick()
{
   ...
   //--- Status print
   Print(StringFormat("FANO_INV | Dir:%+d | Conf:%.3f | Triple:T%d | J:%.3f C:%.3f X:%.2f | Grid:%d/%d | Lot:%.4f | DD:%.2f%%/%.2f%%",
